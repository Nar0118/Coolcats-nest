...
export class PetManagerService {
   ...
   * @param to
   */
  public async onPetTransferEvent(
    tokenId: number,
    from: string,
    to: string,
  ): Promise<void> {
    ...
    if (from === Util.BLACK_HOLE) {
      try {
        ...
        const coolpetsRepository: Repository<Coolpets> =
          getRepository<Coolpets>(Coolpets);
        let coolpetRecord: Coolpets | undefined =
          await coolpetsRepository.findOne({
            where: {
              token_id: tokenId,
            },
          });
        ...
        petMetadata.attributes.forEach(
          (val: { trait_type: string; value: string }) => {
        ...
        console.log(
          `Successfully created coolpets record for PET ID: ${tokenId}`,
        );
        console.log(
          '===========================================================',
        );
        ...
        console.log(
          `Unable to move image / metadata for MINT of PET ID: ${tokenId}`,
        );
        console.log(
          '===========================================================',
        );
   ...
   * @param stage
   */
  public async transformPet(petTokenId: number, stage: EStage): Promise<any> {
    ...
    try {
      ...
      await new Promise<any>((resolve, reject) => {
        const imageParams = {
          ...
          CopySource: `/${
            Environment.env.AWS_S3_PRIVATE_PET_METADATA_IMAGE_BUCKET
          }/${Config.PET_STAGE_PATHS[stateKey as string].image}`,
          Key: `${Config.PET_IMAGE_PATH}${petTokenId.toString()}.png`,
        };
        this.s3.copyObject(
          imageParams,
          (err: AWSError, response: CopyObjectOutput) => {
            if (!err) {
              resolve(response);
            } else {
              reject(err);
      ...
      Util.invalidateCloudfront([
        `/${Config.PET_IMAGE_PATH}${petTokenId.toString()}.png`,
      ]);
      ...
      petMetadata.description =
        Config.PET_STAGE_PATHS[stateKey as string].description;
      petMetadata.attributes[0] = {
        trait_type: 'Stage',
        value: Config.PET_STAGE_PATHS[stateKey as string].stageName,
      };
      petMetadata.image = `https://${
        Environment.env.AWS_S3_PUBLIC_PET_METADATA_IMAGE_BUCKET
      }/${Config.PET_IMAGE_PATH}${petTokenId.toString()}.png`;
      ...
      console.log(
        `------- Failed processing metadata for token: ${petTokenId} stage ${stage}`,
      );
      throw err;
    }
  }
   ...
   * @param petItem
   */
  public async addInteractionToPublicMetadata(
    petTokenId: number,
    petItem: PetItem,
  ): Promise<void> {
   ...
   * @private
   */
  public async transformToFinalForm(petTokenId: number): Promise<void> {
    ...
    if (!coolpet) {
      throw new Error(
        `transformToFinalForm failed to find pet with token id of ${petTokenId}`,
      );
    }
    ...
    if (elements.length > 1) {
      ...
      if (!element) {
        throw new Error(
          `Unexpected undefined selected element type for coolpet id: ${coolpet.id} [${interactions.length} interactions, ${elements.length} tied element types]`,
        );
      }
    } else {
      element = elements[0];
    ...
    const totalVal: number = Object.values(stats).reduce((a, b) => {
    ...
    const ffMetadata: any = await this.readPrivateFinalFormMetadata(
      petTokenId,
      element,
    );
   ...
   * @private
   */
  private async writePublicPetMetadata(
    petTokenId: number,
    petMetadata: any,
  ): Promise<void> {
    await new Promise<any>((resolve, reject) => {
      ...
      this.s3.putObject(
        metadataParams,
        (err: AWSError, response: PutObjectOutput) => {
          if (!err) {
            resolve(response);
          } else {
            reject(err);
    ...
    Util.invalidateCloudfront([
      `/${Config.PET_METADATA_PATH}${petTokenId.toString()}`,
    ]);
  }
   ...
   * @private
   */
  private async writePublicPetImage(
    petTokenId: number,
    elementType: string,
  ): Promise<any> {
    const imagePath = `${elementType}/`;
    const tokenOffset: number = this.tokenOffsetFromElementType(elementType);
    const filePrefix = `${(tokenOffset + petTokenId).toString()}`;
    ...
    if (Environment.env.MODE === 'prod') {
      const thumbnailPath = `${elementType}-thumbnail/`;
    ...
    if (Environment.env.MODE === 'prod') {
      Util.invalidateCloudfront([
        `/${Config.PET_THUMBNAIL_PATH}${petTokenId.toString()}.png`,
      ]);
    }

    const imageFullPath = `https://${Environment.env.AWS_S3_PUBLIC_PET_METADATA_IMAGE_BUCKET}/${imageParams.Key}`;
    const thumbnailFullPath: string =
      Environment.env.MODE === 'prod'
        ? `https://${Environment.env.AWS_S3_PUBLIC_PET_METADATA_IMAGE_BUCKET}/${
            Config.PET_THUMBNAIL_PATH
          }${petTokenId.toString()}.png`
        : '';
   ...
   * @private
   */
  private async readPrivateFinalFormMetadata(
    petTokenId: number,
    elementType: string,
  ): Promise<any> {
    // Read existing metadata from private bucket for the given token ID and element type
    const path = `${elementType}-metadata/`;
    const tokenOffset: number = this.tokenOffsetFromElementType(elementType);
    const filePrefix = `${(tokenOffset + petTokenId).toString()}`;
    const privateMetadataString = await new Promise<GetObjectOutput>(
      (resolve, reject) => {
        const metadataParams = {
        ...
        this.s3.getObject(
          metadataParams,
          (err: AWSError, response: GetObjectOutput) => {
            if (!err) {
              resolve(response);
            } else {
              reject(err);
    ...
    try {
      isOk =
        privateMetadata.hasOwnProperty('name') &&
        typeof privateMetadata.name === 'string' &&
        privateMetadata.hasOwnProperty('description') &&
        typeof privateMetadata.description === 'string' &&
        privateMetadata.hasOwnProperty('attributes');
      if (isOk) {
        privateMetadata.attributes.forEach((val: any) => {
          isOk =
            isOk &&
            val.hasOwnProperty('trait_type') &&
            typeof val.trait_type === 'string';
          isOk =
            isOk &&
    ...
    if (!isOk) {
      throw new Error(
        `Private metadata for pet ID: ${petTokenId} type ${elementType} is not in the correct format`,
      );
    }
   ...
   * @private
   */
  private async readPublicPetMetadata(petTokenId: number): Promise<any> {
    // Read existing metadata from public bucket for the given token ID
    const s3StageMetadata = await new Promise<GetObjectOutput>(
      (resolve, reject) => {
        const metadataParams = {
          Bucket: Environment.env.AWS_S3_PUBLIC_PET_METADATA_IMAGE_BUCKET,
